...
Then pass these objects through the pipeline to the New-ADFineGrainedPasswordPolicy cmdlet to create the fine grained password policy objects.
...
If enabled, the password must contain three of the following five character types:
...
The following examples show how to use these two methods to create a new fine-grained password policy object.
...
### None or Microsoft.ActiveDirectory.Management.ADFineGrainedPasswordPolicy
Returns the new fine grained password policy object when the PassThru parameter is specified.
