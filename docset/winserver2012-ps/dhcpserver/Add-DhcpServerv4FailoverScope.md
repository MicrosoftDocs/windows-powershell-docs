...
online version: https://learn.microsoft.com/powershell/module/dhcpserver/add-dhcpserverv4failoverscope?view=windowsserver2012-ps&wt.mc_id=ps-gethelp
schema: 2.0.0
---
...
```
Add-DhcpServerv4FailoverScope [-Name] <String> [-ScopeId] <IPAddress[]> [-AsJob] [-CimSession <CimSession[]>]
...
The failover relationship to which the scope is being added and the scopes being added to it, should it exist.
...
### EXAMPLE 1
```
PS C:\>Add-DhcpServerv4FailoverScope -ComputerName dhcpserver.contoso.com -Name SFO-SIN-Failover -ScopeId 10.10.10.0, 20.20.20.0
```

This example adds the scopes 10.10.10.0 and 20.20.20.0 to the failover relationship "SFO-SIN-Failover.
The specified scopes and the failover relationship should be present on the DHCP server service running on the computer named dhcpserver.contoso.com.
...
### -Name
Specifies the name of the failover relationship to which the scopes are being added.
...
### -ScopeId
Specifies the scope identifiers (IDs), in IPv4 address format, which are to be added to the failover relationship.
